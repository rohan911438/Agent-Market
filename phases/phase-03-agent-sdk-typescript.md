...
`packages/agent-sdk` (`@agentmarket/agent-sdk`) — the demand-side client. `AgentMarketClient` runs the 402 → pay → 200 loop automatically behind a fluent, thenable `.call()` API:
...
- **The SDK depends on `@agentmarket/shared-types` for wire types.** That's fine inside this monorepo; if this package is ever actually published to npm standalone, that dependency needs resolving first (either publish shared-types too, or fork the ~5 types the SDK actually uses).
