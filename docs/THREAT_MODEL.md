...
| 12 | Wallet address spoofing to claim the higher rate-limit tier | The wallet tier requires `Wallet.isVerified = true` in the database, which is only set after a real settled payment from that address — an unverified claim via `X-Wallet-Address` alone stays on the anonymous tier. |
