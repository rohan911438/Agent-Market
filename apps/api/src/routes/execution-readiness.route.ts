...
import { ExecutionReadinessResponseSchema, SymbolSchema, TimeframeSchema, type ExecutionReadinessResponse } from '@rohankumar4179/shared-types';
...
export const ExecutionReadinessQuerySchema = z.object({
  symbol: SymbolSchema,
  timeframe: TimeframeSchema.optional(),
});
export type ExecutionReadinessQuery = z.infer<typeof ExecutionReadinessQuerySchema>;
...
export async function executionReadinessHandler(
  ctx: AppContext,
  { query, request }: { query: ExecutionReadinessQuery; request: FastifyRequest },
...
export function registerExecutionReadinessRoute(server: FastifyInstance, ctx: AppContext): void {
  registerMeteredRoute({
    ...
    handler: (args) => executionReadinessHandler(ctx, args),
  });
}
